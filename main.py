...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash("You're already a registered user")
...
def login():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data).first()
        if not user:
            flash(f"{form.email.data} is not registered!")
            return redirect(url_for('login'))
...
def logout():
    logout_user()
    return redirect(url_for('get_all_posts', year=year, current_user=current_user))
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", year=year, form=form, current_user=current_user)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", year=year, form=edit_form, current_user=current_user)
